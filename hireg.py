...
def writestrn(buf: BinaryIO, s: str, n: int):
    ret = writememn(buf, s.encode(), n - 1)
    buf.write(b'\0' * (n - ret))
    return n
...
class Register(NamedTuple):
    ...
    def value_len_v120(self):
        return ((self.value >> self._shift).bit_length() + 7) >> 3
...
class RegModule(NamedTuple):
    ...
    def from_ini(cls, obj: Mapping[str, str]):
        ...
        return cls(
            ...
            to_bool(obj.get('boot', '1')), to_bool(obj.get('wakeup', '1')),
            to_bool(obj.get('chip_normal', '1')),
            to_bool(obj.get('chip_ca', '1')), regs)
    ...
    def load_cfg(cls, buf: BufferedReader):
        ...
        return cls(
            RegModuleType.NORMAL, bool(flags & 2), bool(
                flags & 1), False, True,
            regs), None
